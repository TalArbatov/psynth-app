...
function bindOscControls(voiceIndex, prefix) {
    ...
    // Waveform preview
    const wfPreview = createWaveformPreview(document.getElementById(`waveform${voiceIndex + 1}-preview`));
    wfPreview.setWaveform(waveformSel.value);
    // Detune knob (already in HTML as canvas + hidden input)
    const knob = createKnob(document.getElementById(`detune${voiceIndex + 1}-knob`), detuneInput, {
...
function bindLFOControls(oscIndex, lfoIndex) {
    ...
    const lfoWfPreview = createWaveformPreview(document.getElementById(`lfo-waveform-${o}-${l}-preview`));
