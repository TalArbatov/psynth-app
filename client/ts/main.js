...
// @ts-nocheck
const audio_engine_js_1 = require("./modules/audio-engine.js");
const adsr_graph_js_1 = require("./modules/adsr-graph.js");
const piano_keyboard_js_1 = require("./modules/piano-keyboard.js");
const waveform_display_js_1 = require("./modules/waveform-display.js");
const lfo_js_1 = require("./modules/lfo.js");
const sync_js_1 = require("./modules/sync.js");
const fx_chain_js_1 = require("./modules/fx/fx-chain.js");
const waveform_preview_js_1 = require("./modules/waveform-preview.js");
const knob_js_1 = require("./modules/knob.js");
// --- Knob instances registry ---
const knobInstances = new Map();
// --- Sync ---
const sync = (0, sync_js_1.createSync)();
// --- Audio engine (2 oscillator voices) ---
const engine = new audio_engine_js_1.AudioEngine(2);
// --- FX Chain ---
const fxChain = (0, fx_chain_js_1.createFXChain)(engine.audioCtx);
...
const lfos = [0, 1].map(() => Array.from({ length: 4 }, () => new lfo_js_1.LFO()));
...
const adsrGraphs = [
    (0, adsr_graph_js_1.createADSRGraph)(document.getElementById('adsr1-canvas'), document.getElementById('adsr1-values'), engine.voices[0].adsr, () => {
        const a = engine.voices[0].adsr;
        sync.send({ t: 'adsr', n: 0, a: a.a, d: a.d, s: a.s, r: a.r });
    }),
    (0, adsr_graph_js_1.createADSRGraph)(document.getElementById('adsr2-canvas'), document.getElementById('adsr2-values'), engine.voices[1].adsr, () => {
        const a = engine.voices[1].adsr;
        sync.send({ t: 'adsr', n: 1, a: a.a, d: a.d, s: a.s, r: a.r });
    })
];
// --- Piano keyboard ---
const keyboard = (0, piano_keyboard_js_1.createPianoKeyboard)(document.getElementById('keyboard'), {
    onNoteOn: freq => {
        engine.noteOn(freq);
...
// --- Waveform display ---
const waveform = (0, waveform_display_js_1.createWaveformDisplay)(document.getElementById('waveform-canvas'), engine.analyser);
...
function upgradeAllSliders() {
    ...
    sliders.forEach(slider => {
        ...
        const knob = (0, knob_js_1.createKnob)(canvas, hidden, {
...
function animate() {
    ...
    (_a = knobInstances.get('master-volume')) === null || _a === void 0 ? void 0 : _a.setValue(modVol);
...
Promise.resolve().then(() => __importStar(require('./modules/filter-graph.js'))).then(({ createFilterGraph }) => {
    const fg1 = createFilterGraph(document.getElementById('filter1-canvas'), document.getElementById('filter1-values'), engine.voices[0], () => {
        sync.send({
            t: 'filter', n: 0,
    ...
    drawList.push(fg1);
    const fg2 = createFilterGraph(document.getElementById('filter2-canvas'), document.getElementById('filter2-values'), engine.voices[1], () => {
        sync.send({
            t: 'filter', n: 1,
...
function bindOscControls(voiceIndex, prefix) {
    ...
    // Waveform preview
    const wfPreview = (0, waveform_preview_js_1.createWaveformPreview)(document.getElementById(`waveform${voiceIndex + 1}-preview`));
    wfPreview.setWaveform(waveformSel.value);
    // Detune knob (already in HTML as canvas + hidden input)
    const knob = (0, knob_js_1.createKnob)(document.getElementById(`detune${voiceIndex + 1}-knob`), detuneInput, {
...
function bindLFOControls(oscIndex, lfoIndex) {
    ...
    const lfoWfPreview = (0, waveform_preview_js_1.createWaveformPreview)(document.getElementById(`lfo-waveform-${o}-${l}-preview`));
    ...
    syncToggle.addEventListener('click', () => {
        ...
        (_a = knobInstances.get(`lfo-rate-${o}-${l}`)) === null || _a === void 0 ? void 0 : _a.setEnabled(!lfo.bpmSync);
    });
...
function initDragDrop() {
    ...
    dropTargets.forEach(target => {
        ...
        target.addEventListener('drop', (e) => {
            ...
            if (type === 'filter' && parseInt(oscNum) !== oscN)
                return;
            // Toggle target
...
sync.onMessage((msg) => {
    ...
    if (msg.t === 'ctrl') {
        ...
        (_a = knobInstances.get(msg.id)) === null || _a === void 0 ? void 0 : _a.setValue(parseFloat(msg.v));
